...
class CalFitCostFunctor {
public:
  
  CalFitCostFunctor(const std::vector<WeightDataDay>& wdv)
    : m_wdv(wdv) {}
...
private:
  ...
  static std::vector<T> estDailyLosses(std::vector<WeightDataDay>& days, 
	...
	return r;
      r.push_back(T(0));
#ifdef DEBUG_OUTPUT
      std::cout << "Daily Losses computed with cal " << cal[0] << std::endl;
      std::cout << 0 << " " << r[0] << std::endl;
#endif
      for (int i = 1; i < days.size(); i++)
      {
	r.push_back(estDailyLoss(T(days[i-1].m_weight),
				 T(days[i-1].m_actEstimate),
				 T(cal[0])));
#ifdef DEBUG_OUTPUT
	std::cout << i 
		  << " " << r[i] 
